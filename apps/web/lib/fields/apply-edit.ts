 ...
 * field_edit_proposals has no reviewer column, so who applied it is not
 * recorded on the row.
 */
export async function applyFieldEditProposal(
  proposalId: string,
  ...
  await db
    ...
    .where(eq(fieldEditProposals.id, proposalId))
  // The "your edit was applied" email. Off when the submitter is the admin who
  // just applied it.
  if (options.notifySubmitter !== false) await notifyFieldEditApplied(proposalId, changed)
  return {}
}
