...
/**
 * Off-season event candidate decisions. One body each, shared by the admin
 * buttons in app/admin/event-listings/candidates/actions.ts and by
 * /api/internal/queue-decisions. The callers own auth and cache revalidation
 * (the queue and /admin/event-listings); every other side effect (the listing
 * insert, source counters, closing twins, the roster refresh) is here so both
 * callers get it.
 */
...
export async function loadEventCandidate(candidateId: string) {
  const db = getDb()
  const [row] = await db
    .select()
 ...
 */
export async function findEventListing(ref: string) {
 ...
 * inventing one. The reviewer confirms it.
 *
 ...
 */
export async function acceptEventCandidateBody(
  candidateId: string,
 ...
 */
export async function attachEventCandidateBody(candidateId: string, listingRef: string): Promise<{ error?: string }> {
  const candidate = await loadEventCandidate(candidateId)
  if (!candidate) return { error: 'Candidate not found.' }

  const listing = await findEventListing(listingRef)
 ...
 */
export async function suppressEventCandidateBody(candidateId: string, reason: string): Promise<{ error?: string }> {
 ...
 */
export async function markEventCandidateDuplicateBody(candidateId: string, listingRef: string): Promise<{ error?: string }> {
  const candidate = await loadEventCandidate(candidateId)
  if (!candidate) return { error: 'Candidate not found.' }
