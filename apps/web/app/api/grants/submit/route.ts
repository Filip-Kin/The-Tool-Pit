...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await createGrantSubmission({
      ...
      submitted: {
        deadlineAt: str(form, 'deadlineAt'),
        deadlineType: str(form, 'deadlineType'),
        awardMax: str(form, 'awardMax') ? Number(String(str(form, 'awardMax')).replace(/[^0-9.]/g, '')) : undefined,
        programs: str(form, 'programs'),
        regions: str(form, 'regions'),
        eligibility: str(form, 'eligibility'),
        effortLevel: str(form, 'effortLevel'),
        contactEmail: str(form, 'contactEmail'),
        howKnown: str(form, 'howKnown'),
      },
