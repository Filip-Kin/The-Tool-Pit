...
const EVENT_FORM: ListingFormSpec = {
  ...
  fields: [
    ...
    },
    {
      ...
      label: 'Each day is its own 1-day event',
      kind: 'checkbox',
      group: 'when',
      hint: 'One event over two days where each day is a separate tournament with its own team list, its own TBA code, and sometimes its own sign-up form. Team slots then count per day.',
    },
    ...
    {
      key: 'registrationUrlDay2',
      label: 'Registration link, day 2',
      kind: 'url',
      group: 'links',
      maxLength: 500,
      wide: true,
      showWhen: { field: 'parallelDivisions', equals: 'true' },
      hint: 'Only when day 2 has its own sign-up form. Leave blank when one form covers both days.',
    },
    ...
    },
    {
      ...
      hint: 'One team per line, e.g. 254. For a second robot from the same team (a B team), add the letter right after the number: 4145B. When each day is its own event, this is DAY 1.',
    },
    {
      key: 'manualTeamListTextDay2',
      label: 'Teams, day 2',
      kind: 'textarea',
      group: 'teamlist',
      maxLength: 20_000,
      ...
      wide: true,
      showWhen: { field: 'parallelDivisions', equals: 'true' },
      hint: 'Day 2 of a two-1-day-events listing, one team per line. Read only when the team list comes from you (not scraped).',
    },
    {
      key: 'notes',
      label: 'Notes',
